...
- [ ] 3.6 Change the workflow to read-only PR validation and serialized deployment after successful main push checks. Render the exact checked SHA, reject obsolete revisions, and fail on freshness-query errors. Exercise real PR validation with the separate identity.
- [ ] 3.7 After review and merge, confirm native checks precede apply and the live policy equals that checked revision's rendered policy. Verify failed/PR check completion cannot deploy.
...
- [ ] 5.7 After activation, use root's new SSH endpoint to confirm the non-interactive `nix-daemon` path and record it in the runbook.
...
- [ ] 6.1 Run the nonce-based `tailnet-builder-check` over OpenSSH; confirm a live build reports `arm64`, `macbook-pro`, and the measured Tailscale path.
- [ ] 6.2 Build the Darwin system check from `korolev`; confirm the remote builder runs and its output enters the local store.
- [ ] 6.3 Run all-system checks on the combined revision and compare Darwin check derivations with native evaluation of that same revision.
- [ ] 6.4 Run the packaged Darwin build-plan inspection from Linux through native SSH and confirm its output and exit status match a local Darwin invocation.
