...
- [ ] 1.1 Create the tailnet, enable MagicDNS, and record the tailnet ID in the runbook; confirm that the admin console shows MagicDNS enabled and zero nodes.
- [ ] 1.2 Create a federated identity for `glockyco/nix-config` with the `policy_file` scope and add `TS_OAUTH_ID`, `TS_AUDIENCE`, and `TS_TAILNET` as repository secrets; confirm with `gh api repos/glockyco/nix-config/actions/secrets` that exactly those three names exist.
