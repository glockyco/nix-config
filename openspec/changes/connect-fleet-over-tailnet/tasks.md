...
- [x] 5.3 Add `packages/tailnet-known-hosts.nix` and `packages/tailnet-known-hosts-tests.nix` per design decision 4, wire the test as `tailnetKnownHostsCommand`, and confirm that the fixture for a matching peer prints one `known_hosts` line per key and the other two fixtures exit nonzero.
