...
`modules/darwin/tailscale.nix` enables `services.tailscale` and adds one launchd daemon, `tailscaled-set`, that runs `tailscale set --ssh --advertise-tags=tag:macbook-pro` after authentication completes. Its packaged runner waits until `tailscale status --json` reports `BackendState` as `Running`; command success only proves that the local socket exists, and an initial `tailscale up` resets preferences applied while the state is `NeedsLogin`. launchd retries the runner when `tailscale set` fails. This mirrors the NixOS `tailscaled-set` option boundary while adding the ordering that first-time macOS authentication requires, because nix-darwin has no `extraSetFlags`. The module declares the option `services.tailscale.extraSetFlags` with the NixOS type and description, and one task tracks the upstream nix-darwin pull request that adds the same option. When upstream ships compatible authenticated-state ordering, the repository option and runner are deleted while the declaration stays. `services.openssh.enable` stays unset, which leaves Remote Login off.
...
`modules/nixos/tailscale.nix` enables `services.tailscale` with `extraSetFlags = [ "--shields-up" "--advertise-tags=tag:korolev" ]`, `disableTaildrop = true`, and `openFirewall = false`. `modules/nixos/nix.nix` sets `nix.distributedBuilds = true`, `nix.settings.builders-use-substitutes = true`, and one `nix.buildMachines` entry: `hostName = "macbook-pro"`, `sshUser = "glockyco"`, `system = "aarch64-darwin"`, `protocol = "ssh-ng"`, `maxJobs` and `speedFactor` from the Mac's core count, `supportedFeatures = [ "big-parallel" ]`, no `sshKey`. The entry derives its host name and user from the Mac's host declaration through `inputs.self.darwinConfigurations.macbook-pro.config.host`, so a rename changes it in one place.
...
- `korolevIsolation` extended: `services.tailscale.enable`, `openFirewall == false`, `disableTaildrop`, `--shields-up` present and `--ssh` absent in `extraSetFlags`, `networking.firewall.allowedUDPPorts == []`, `nix.distributedBuilds`, exactly one build machine named `macbook-pro` with no `sshKey`.
- `macbookProTailnet`: `services.tailscale.enable`, `--ssh` in the Mac's set flags, `services.openssh.enable == null`, `glockyco` in `trusted-users`.
