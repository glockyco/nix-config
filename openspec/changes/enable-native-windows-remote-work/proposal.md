...
- Install `psmux` for terminal convenience and verify rendering, resize, Unicode, paste and loopback-only control endpoints. Live persistence across a lost SSH connection is out of scope: the owner does not want it, so no detachment or abrupt-disconnect trial is accepted, and the procedure claims no surviving process.
