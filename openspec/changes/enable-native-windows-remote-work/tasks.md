...
- [x] 3.3 Install the selected psmux release and create a named session using upstream commands. Verify terminal rendering, resize, Unicode, paste, and local-only multiplexer control endpoints without unsafe mouse overrides. Interrupt handling and the live persistence trials are dropped by owner decision, recorded in the scheduling notice above; psmux stays installed as a convenience, not an accepted capability.
