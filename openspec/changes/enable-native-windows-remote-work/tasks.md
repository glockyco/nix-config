...
- [ ] 3.3 Install the selected psmux release and create a named OMP session using upstream commands. Verify terminal rendering, resize, Unicode, paste, interrupt handling, and local-only multiplexer control endpoints without unsafe mouse overrides. Rendering, resize, Unicode, paste and loopback-only endpoints pass. Verify interrupt handling during the persistence trial in 3.4, where a terminal-attached client is present anyway; injected keys are not a terminal interrupt and prove nothing.
