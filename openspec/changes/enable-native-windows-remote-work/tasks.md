...
- [ ] 4.3 Provide and exercise RDP clients on Pro, Air, and Korolev without modifying Korolev's Windows host. The Pro's client is declared and activated (Windows App 11.4.0); connecting, comparing the certificate fingerprint, and the disconnect and reconnect check need the operator. Start a harmless terminal task, disconnect, and reconnect as the same Windows user. Verify the task and session survived and inspect effective disconnected-session limits.
