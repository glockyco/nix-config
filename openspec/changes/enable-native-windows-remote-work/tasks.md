...
- [ ] 5.5 Verify Korolev still rejects incoming fleet connections and run the existing live Mac builder and command-status checks. Preserve previous generations and leave unrelated `connect-fleet-over-tailnet` gates unchanged. Inbound rejection, the live remote build, remote store status, command status and retained generations all pass. Two `tailnet-builder-check` failures are recorded: a no-reply ping under Korolev's earlier unstable network, and afterwards a relayed pong rejected because the check demanded a direct path. The second was a checker defect, now fixed with `--until-direct=false`; re-running the fixed check on Korolev is the remaining step.
