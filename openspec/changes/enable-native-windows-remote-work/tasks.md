...
- [ ] 5.5 Verify Korolev still rejects incoming fleet connections and run the existing live Mac builder and command-status checks. Preserve previous generations and leave unrelated `connect-fleet-over-tailnet` gates unchanged. Blocked: inbound rejection, the live remote build, remote store status, command status and retained generations all pass, but `tailnet-builder-check` exits 1 because its final tailnet ping to the Pro times out. The cause is undiagnosed and belongs to `connect-fleet-over-tailnet`; this task stays open rather than accepting a failing check.
