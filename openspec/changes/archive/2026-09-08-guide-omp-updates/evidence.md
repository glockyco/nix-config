...
## Herdr prerequisite clarification

On 2026-09-08, the live session's Bash command reported `HERDR_ENV=1` and `HERDR_PANE_ID=w1:p2`. Python Eval's `env(...)` returned `None` for both markers. This establishes a tool-environment mismatch in that session, not the cause of the earlier user's blocker. The mismatch was reported through automated QA; this guidance change does not fix the harness.

Three fresh, tool-disabled model rehearsals consumed the revised prerequisite procedure. With an absent Eval marker and Bash value `1`, the response followed `herdr --skill` rather than reporting a blocker. With an absent Bash marker, it prohibited pane control and marker fabrication, continued the available managed-browser smoke, and requested an operator handoff for the remaining Herdr checks. With a command-start error, it distinguished the error from an absent marker and withheld pane control. Inputs, procedure text, and full responses are retained under `herdr_prerequisite` in `verification.json`.

These responses verify guidance decisions, not execution of the stated smoke checks. No production update, activation, publication, harness repair, or additional host acceptance was performed for this clarification.

An isolated worktree based on `62a4e8a86847f43f6e3367ab6060a472059a905e` contained only these eight task-owned file changes. `nix fmt` made no changes; `nix fmt -- --fail-on-change`, strict validation of `guide-omp-updates`, and `nix flake check --print-build-logs` passed. The flake check covered the current `x86_64-linux` system; Darwin was explicitly omitted. Final completion annotations received a further formatting and strict-validation check. This documentation clarification does not repeat the earlier both-host runtime smoke.
