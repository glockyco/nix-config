...
The native `kbdneo` layout must copy checksum-pinned DLLs into `System32` and `SysWOW64` and register `HKLM\SYSTEM\CurrentControlSet\Control\Keyboard Layouts\b0000407`. A second Administrator script owns only those paths, verifies every downloaded payload, and requires a full Windows restart. The interactive-user document then selects input tip `0407:b0000407`. ReNeo stays installed per user and runs in extension mode to supply the Neo layers that the native driver does not implement. Its user-scope logon launcher requests the separate Administrator credential and starts the exact per-user executable with `RunAs`; the resulting process covers both ordinary and elevated applications. The UAC secure desktop remains native-driver-only because normal processes cannot inject into it.

The upstream Task Scheduler recipe assumes that the interactive account belongs to `Administrators` and has a linked elevated token. `JGlock` is a standard user, while elevation authenticates the separate `Administrator` account. A highest-privilege task under `JGlock` therefore stays unelevated; a background task under `Administrator` runs outside the interactive session and cannot install keyboard hooks there. The explicit credential prompt at each logon is the narrow effective option. It stores no password and does not change group membership.
...
Select the built-in Windows dark appearance and the dark Bloom wallpaper, but disable transparency and animation effects. Preserve the current standard blue accent by leaving the custom accent-palette binary data untouched. The appearance resource names and tests the exact per-user registry values, applies the wallpaper through `SystemParametersInfo`, and broadcasts the supported setting-change notification. A separate resource uses `SPI_GETCLIENTAREAANIMATION` and `SPI_SETCLIENTAREAANIMATION`, the supported Windows accessibility API, instead of editing the opaque `UserPreferencesMask` binary value.

Put `en-GB` first in the user's preferred language list and set it as the Windows UI override, but leave its input-method list empty. Preserve the existing `de-DE` and `de-AT` input methods and keep native Neo as the explicit default. `Set-WinUserLanguageList` restores the standard UK keyboard for a directly empty English entry. Supplying the German Neo tip as a temporary English-entry input makes Windows discard that language-mismatched tip during normalization without adding the UK keyboard. This keeps Windows and PowerToys in English while Neo becomes the first usable input method after sign-in. Set the short date to ISO 8601 `yyyy-MM-dd` instead of inheriting the British `dd/MM/yyyy` format.

The installed Widgets build uses the Austrian country or region to select German Microsoft-hosted content even when its interface culture is `en-GB`. It exposes no separate content-language control. Preserve the Austrian region and accept German widget and taskbar-weather content rather than changing regional Microsoft services.
...
| Neo2 keyboard layout          | Native `kbdneo` plus elevated ReNeo     | The native driver covers sign-in, UAC, and elevated windows. ReNeo installs per user in extension mode; a logon `RunAs` prompt gives its higher layers access to ordinary and elevated applications. UAC remains native-driver-only.                                                                                                                                                                                                                                                                                                                          |
...
- **The native driver does not implement every higher Neo layer.** → Start ReNeo with the separate Administrator credential at each logon so one process covers ordinary and elevated applications. UAC's secure desktop still receives only the native base layout; it does not admit ReNeo injection.
