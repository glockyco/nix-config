...
- [x] 4.3 Complete the activated C# and Markdown smoke through the installed OMP wrapper on both hosts, using existing project structure from actual repositories. Keep OMP responsive during project loading; initial semantic results may be incomplete. Require diagnostics, cross-file definition, references, rename, and repeated post-edit diagnostics after project loading. Crashes, lost edits, and persistent failures fail acceptance. Do not add hidden retries or restarts, and do not relabel incomplete startup responses as successful requests. Recover the reported Darwin smoke evidence or run a clearly labeled new acceptance session. Record host, repository, executable/plugin/server versions, requests, results, and file edits. Merge and activation have occurred; do not reactivate an unchanged generation. Keep the previous generation until acceptance passes.
...
The following chronological record preserves the observations and assessments made before the owner revised the acceptance contract. Statements that task 4.3 remains open or Linux acceptance failed describe the earlier immediate-readiness criterion, not the current status. The final owner decision below supersedes those assessments without changing the recorded request results.
...
### Startup barrier control: 2026-09-05

A bounded direct-server control used the installed Roslyn `5.12.0-1.26426.8` and a disposable clone of HotRepl revision `7d9c49ed57f5436bf7b258207ea14236333b4ac6`. The existing development shell restored Core in locked mode. The existing solution was unchanged; only the `HostInfo.Name` initializer was changed to `123`.

The corrected control sent one diagnostic request at 15.037 seconds, while the solution-loading progress token remained active and project initialization was incomplete. That request returned only IDE0005, without CS0029. Roslyn sent `workspace/projectInitializationComplete` at 15.379 seconds. A second, explicitly scheduled diagnostic control after that notification returned CS0029 for the unchanged document. These two requests compare loading states; they are not an acceptance retry or a wrapped-session pass.

The first control had a request-ID bookkeeping error: it confused a response to a server request with its own planned second request. It reached the 90-second experiment deadline without sending that request. Its retained trace independently records active loading at 15.038 seconds, IDE0005 without CS0029 at 15.829 seconds, and project initialization completion at 21.363 seconds. The corrected run preserves this failure record rather than treating it as a successful first run.

[OMP 18.1.10 client source](https://github.com/can1357/oh-my-pi/blob/v18.1.10/packages/coding-agent/src/lsp/client.ts) sets `PROJECT_LOAD_TIMEOUT_MS` to 15,000 and unconditionally resolves `projectLoaded` when that timer expires. The timer does not check active progress tokens. Executing the exact startup-barrier block with a controlled timer confirmed that it resolves without any progress-end event. [Diagnostic and semantic dispatch](https://github.com/can1357/oh-my-pi/blob/v18.1.10/packages/coding-agent/src/lsp/tool.ts) use that barrier. This reproduces an incomplete initial compiler diagnostic at the bounded-wait boundary. The earlier classification as a client readiness defect is retracted under the owner-approved nonblocking startup contract. It does not trace the earlier wrapped run or independently establish the cause of its missing definition.

The earlier recommendation to change OMP's readiness barrier is withdrawn. A bounded wait preserves responsiveness; incomplete startup semantics do not establish a defect against the accepted contract. No client patch, timeout override, retry, package update, or activation was applied.

### Owner-approved acceptance decision: 2026-09-05

The owner rejected immediate semantic completeness as an acceptance requirement. OMP must remain responsive while language servers load. Initial diagnostic and navigation responses may be incomplete. After project loading, diagnostics, navigation, references, cross-file edits, and repeated post-edit diagnostics must work correctly. Crashes, lost edits, and persistent failures remain unacceptable.

Task 4.3 is complete against this revised contract using the existing wrapped-session evidence, not the direct-server control or a new acceptance run. Markdown operations passed on both hosts. The real HotRepl workflow returned all seven expected results on Darwin. Linux's initial diagnostic and definition responses remain incomplete; subsequent references, cross-file rename, both post-edit diagnostic requests, and renamed-symbol definition succeeded. The inspected file diffs confirmed the edits and retained compiler error. No request cancellation or server restart was reported in those HotRepl sessions.

The original Linux 5/7 result is preserved; it is not relabeled 7/7. Acceptance changed because the owner corrected the startup expectation, not because another run repaired the historical responses. No further reproduction or implementation change is warranted for this startup behavior. All 12 prebuilt-change tasks are complete. Archiving remains a separate action; the tailnet change's four optional tasks remain deferred and incomplete.
