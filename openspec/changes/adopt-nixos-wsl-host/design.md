...
`user@1000.service` fails only while another distribution with the same user ID runs. WSL places no distribution in its own cgroup namespace, so every running distribution targets `/user.slice/user-1000.slice/user@1000.service`, and the second one to start cannot attach. A controlled measurement on `korolev` confirmed the mechanism. The unit fails with `Failed to spawn executor: Device or resource busy` while `Ubuntu-26.04` runs, and it starts one second after that distribution stops. An interactive session does not change the result, and neither does a restart of the unit.

The occupied path gives no direct evidence. The cgroup directory exists and `cgroup.procs` reads empty, because the processes that hold the path belong to another PID namespace. An empty `cgroup.procs` is therefore no proof that the path is free. `wsl.exe --terminate` also returns before the release completes. A distribution that had run for one hour still held the path 70 seconds after that command returned.

The collision also breaks activation. `nixos-rebuild switch` reloads the user units as its last step, and that step needs the user manager. The step runs after the command registers the generation, so a failure leaves a registered generation whose user scope never reloaded. The command prints `user activation for user failed` and returns exit status 4.

A stopped distribution does not repair the failed unit. The user manager stays dead, the user bus socket stays absent, and the switch then fails with `Failed to open dbus connection`. The activation gate therefore needs an observed unit state, not the return value of a termination command. Confirm that `user@1000.service` reports `active` before an activation, and start the unit if it is dead.

Decision 6 requires the Ubuntu distribution to stay registered, because that is what preserves the rollback path. It does not require that distribution to run. Run one distribution at a time during the cutover, and the collision does not arise.

The condition therefore belongs to the side-by-side window, not to the host. Require the clean unit state and a clean activation while no other distribution runs, because a permanent `degraded` state hides real failures.
