...
- Make `applications.nix` the single declaration for every application, including each version policy and the release data of AltSnap and the font. `files.nix` and `font.nix` receive the application list and derive resource metadata and version selectors from it.
- Expose the declaration through `passthru.declaration`: the role set, the application list with version policies, the managed-application identifiers, and the review file names.
...
- Parse every embedded script, both Administrator scripts, and the ReNeo launcher with `pwsh` through `[Management.Automation.Language.Parser]::ParseInput`. Assert the Administrator-script boundary on the parsed variable references and string constants. Drop every substring assertion, including checks for obsolete active-theme-path ownership.
