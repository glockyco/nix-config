...
- Rewrite the architecture document: rename every repository reference to `nix-config`, delete the current-state table and the Resume procedure, delete the per-change delivered-work subsections, move the four per-project migration plans to issues in their owning repositories and keep only the dependency order and the common contract, replace the version-pinned primary references with unversioned links, and record the two decisions of this session that the document must reflect.
