...
- [ ] 2.3 Create one `planning` issue for the scoped WSL batch worker from `2026-08-09-multi-host-fleet-plan.md` section "Boundary on the wife's machine", state in its body that the rest of the record is superseded by `declare-typed-host-options`, `connect-fleet-over-tailnet`, and `key-fleet-by-host`, and confirm with `gh issue list --label planning --json title` that six titles are listed.
...
- [ ] 4.1 Confirm that `connect-fleet-over-tailnet` recorded the reversed `korolev` isolation decision in the decision log and rewrote the `WSL work machine` ownership section, with `grep -c 'declares no SSH server, no other inbound service, and no secret' docs/architecture/personal-omp-environment.md` printing `0` and `grep -c 'remote build' docs/architecture/personal-omp-environment.md` printing at least `1`; if either fails, add the missing text from the archived proposal of that change.
