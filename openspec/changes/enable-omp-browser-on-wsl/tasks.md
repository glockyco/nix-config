...
- [x] 4.4 Run the documented extension installer, create the dedicated Brave profile, and load the unpacked extension. Open one dedicated tab and confirm that an OMP relay session adopts that tab without controlling a Zen tab or another Brave profile.
- [x] 4.5 Restart Windows and sign in. Confirm that Brave and the relay daemon remain stopped until requested, then repeat the relay connection check.
