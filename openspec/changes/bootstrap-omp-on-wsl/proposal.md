...
- After Nix is available, expose one supported command that installs or updates the user profile, reconciles Herdr, and verifies the immutable plugin without a Darwin activation.
...
The change affects the flake outputs, the personal OMP package helpers, Linux checks, operator documentation, and the `personal-omp-workstation` contract. It uses the existing locked OMP, Herdr, OpenSpec, plugin, and language-server inputs. It documents Windows Terminal Stable as a manual host prerequisite, but it does not install or configure Windows applications. It does not activate `nix-darwin` or Home Manager on WSL, and it does not alter the current Mac host.
