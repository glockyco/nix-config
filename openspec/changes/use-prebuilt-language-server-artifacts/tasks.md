...
[OMP 18.1.10 client source](https://github.com/can1357/oh-my-pi/blob/v18.1.10/packages/coding-agent/src/lsp/client.ts) sets `PROJECT_LOAD_TIMEOUT_MS` to 15,000 and unconditionally resolves `projectLoaded` when that timer expires. The timer does not check active progress tokens. Executing the exact startup-barrier block with a controlled timer confirmed that it resolves without any progress-end event. [Diagnostic and semantic dispatch](https://github.com/can1357/oh-my-pi/blob/v18.1.10/packages/coding-agent/src/lsp/tool.ts) use that barrier. This establishes a client readiness defect and reproduces the missing initial compiler diagnostic at its boundary. It does not trace the earlier wrapped run or independently establish the cause of its missing definition.

The appropriate correction belongs in OMP: loading must finish before project-dependent requests proceed, and an expired deadline must report failure rather than readiness. No client patch, timeout override, retry, package update, or activation was applied. Such an upstream correction requires an explicit scope decision under this change's current exclusions. Task 4.3 remains open. The tailnet change also remains unarchived because its four optional tasks are still deferred, not completed.
