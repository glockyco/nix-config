...
| ----------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------- |
| OMP installation                                | Both installed wrappers start and both local verifiers passed                                                                                                  | [Prebuilt acceptance](../../openspec/changes/archive/2026-09-05-use-prebuilt-language-server-artifacts/tasks.md) |
| Activated OMP workflow                          | Ordinary use and language acceptance passed on both hosts under the nonblocking startup contract. Linux initial semantic responses remain incomplete           | [Prebuilt acceptance](../../openspec/changes/archive/2026-09-05-use-prebuilt-language-server-artifacts/tasks.md) |
...
| Restart and isolation checks                    | Restart and live SSH isolation checks passed; employer-internal DNS is not applicable                                                                          | [Tailnet tasks](../../openspec/changes/connect-fleet-over-tailnet/tasks.md)                                      |
...
| C# integration        | Acceptance is complete using the recorded HotRepl workflow on both hosts. Keep the installed client and server unchanged                                                                | Initial semantics may be incomplete during loading. Require correct semantic operations after loading; crashes, lost edits, and persistent failures are unacceptable |
| Tailscale reliability | Restart verification is complete: resolver ownership, public DNS, MagicDNS, connectivity, and SSH passed. Employer-internal DNS is not applicable                                       | Restart and live SSH isolation results are recorded. Preserve the existing network boundaries                                                                        |
...
The archived prebuilt change records completed OMP acceptance. The tailnet change owns network acceptance and remains active for its deferred optional tasks. The owner accepted incomplete startup semantics as the responsiveness tradeoff. The earlier startup-defect classification is withdrawn; the recorded post-load workflow completes C# acceptance. This does not accept crashes, lost edits, or persistent failures.
...
Windows owns Windows Terminal, WSL enablement, employer policy, native application state, and the editor runtime. This repository owns the reviewed Windows declaration and uses Nix to render it; the operator applies that artifact with WinGet and DSC. The declaration installs pinned, user-scope Brave only for browser relay, while Zen remains the interactive browser. The NixOS host `korolev` owns the Linux system scope, user scope, OMP wrapper and plugin, Chromium shared-library ABI, Tailscale service and resolver configuration, Herdr, OpenSpec, and language tools. Tailscale owns writable node authentication and control-plane state. The official oh-my-pi installer owns the user-local OMP executable. OMP owns its downloaded Chromium, caches, profiles, relay extension, and other mutable runtime state. Repositories stay under the Linux home directory, not `/mnt/c`.

The root flake exposes `nixosConfigurations.korolev` for `x86_64-linux`. `nixos-rebuild switch --flake .#korolev` activates the host and retains generation rollback targets. The host declares no SSH server or other inbound service, and shields-up rejects inbound tailnet connections. Its declared Darwin build transport is native OpenSSH over Tailscale. Root owns the dedicated client key outside the Nix store; the repository declares its restricted public authorization and the Mac's verified public host key. The Mac's standalone launchd SSH daemon binds only its full MagicDNS name. Apple Remote Login and Tailscale SSH are disabled by the new configuration. Post-merge activation and root-daemon/PAM acceptance remain tracked in `connect-fleet-over-tailnet`; the unprivileged native smoke is not deployment evidence.
...
- Reverse the earlier decision that endpoint data-loss prevention required `korolev` to have no remote access. Permit outbound tailnet access and Darwin remote builds, but retain no inbound service, shields-up, no open firewall port, and no SSH private key.
- Keep durable managed hosts in typed host declarations and unmanaged peers in shared tailnet data. Treat the borrowed Air as one removable temporary peer; planning issue [#17](https://github.com/glockyco/nix-config/issues/17) owns its result-preservation, revocation, and complete configuration removal boundary.
...
- Replace macOS Tailscale SSH with standard OpenSSH over the tailnet because the deployed server reports success for `exit 23`. Do not repair remote status with a wrapper or assume an unverified upgrade fixes it.
- Permit one root-owned builder private key on `korolev`, outside the repository and Nix store. Declare its public key with `restrict` and pin the Mac's actual OpenSSH host key. The key permits commands as a Nix trusted user; it is not a command sandbox.
- Keep Apple Remote Login disabled. Its launchd socket is not constrained by `sshd_config`'s `ListenAddress`. Manage a standalone native daemon that binds only the Mac's full MagicDNS name and fails without a wildcard fallback when that address is unavailable.
- Preserve shields-up, closed inbound firewall policy, and the temporary Air's independent offboarding boundary. SSH cutover does not weaken Linux isolation or make the Air a durable dependency.
- Separate read-only PR policy validation from main/workflow-constrained policy deployment at the Tailscale provider. Deploy the exact successful main check revision, serialize writes, and skip obsolete revisions. Main protection requires the Linux, Darwin, and policy checks and linear PR history. Single-owner human review remains procedural.
- Activate the Mac only after review and merge from a local administrator terminal. Native transport smoke, provider settings, actual GitHub events, root/PAM acceptance, WSL restart, and disconnected-builder recovery are separate evidence gates. Keep the previous generations; rollback does not restore private keys or tailnet enrollment.
