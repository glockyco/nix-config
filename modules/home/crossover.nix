...

{
  ...
  home.activation.crossoverSteamBottle = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    if [ ! -x ${lib.escapeShellArg "${crossover}/bin/cxbottle"} ]; then
      ...
      # cxbottle leaves a partial directory behind when it fails part-way, so
      # clear it out -- otherwise the next run sees something bottle-shaped and
      # a single failure blocks every retry.
